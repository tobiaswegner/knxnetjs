 ...
 */
export class CEMIPropertyReadReq {
  private buffer: Buffer;
  ...
  constructor(
    ...
    this.buffer.writeUInt8(CEMIMessageCode.M_PROP_READ_REQ, offset++);
  ...
  static fromBuffer(buffer: Buffer): CEMIPropertyReadReq {
    if (buffer.length < 7) {
      throw new Error("Invalid CEMIPropertyReadReq frame: too short");
    }

    const messageCode = buffer.readUInt8(0);
    if (messageCode !== CEMIMessageCode.M_PROP_READ_REQ) {
    ...
    return new CEMIPropertyReadReq(
      interfaceObject,
      objectInstance,
      propertyId,
      numberOfElements,
  ...
  isValid(): boolean {
    return (
      this.buffer.length >= 7 &&
      this.messageCode === CEMIMessageCode.M_PROP_READ_REQ &&
      this.numberOfElements <= 15 &&
      this.startIndex <= 4095
    );
  }
 ...
 */
export class CEMIPropertyReadCon {
  private buffer: Buffer;
  ...
  constructor(
    ...
    this.buffer.writeUInt8(CEMIMessageCode.M_PROP_READ_CON, offset++);
  ...
  static fromBuffer(buffer: Buffer): CEMIPropertyReadCon {
    if (buffer.length < 7) {
      throw new Error("Invalid CEMIPropertyReadCon frame: too short");
    }

    const messageCode = buffer.readUInt8(0);
    if (messageCode !== CEMIMessageCode.M_PROP_READ_CON) {
    ...
    return new CEMIPropertyReadCon(
      interfaceObject,
      objectInstance,
      propertyId,
      numberOfElements,
  ...
  isValid(): boolean {
    return (
      this.buffer.length >= 7 &&
      this.messageCode === CEMIMessageCode.M_PROP_READ_CON &&
      this.numberOfElements <= 15 &&
      this.startIndex <= 4095
    );
  }
